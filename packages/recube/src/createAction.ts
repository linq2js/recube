...
export const createActionFactory = (createState: CreateState) => {
  const createAction = (body?: AnyFunc, middleware: AnyFunc[] = []) => {
    ...
    const resultState = lazyValue<State<any, void>>(() => {
      const [{ dispose }, result] = disposableScope.wrap(() =>
        createState(callInfo.result, { name: '#ACTION_RESULT' }).when(
